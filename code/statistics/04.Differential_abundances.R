...
psOTU_AM <- otu_table(otu_AM, taxa_are_rows = TRUE)
psTAXA_AM <- tax_table(taxa_AM)
...
psOTU_ECM <- otu_table(otu_ECM, taxa_are_rows = TRUE)
psTAXA_ECM <- tax_table(taxa_ECM)
psMETA_ECM <- sample_data(data)
psEM <- phyloseq(psOTU_ECM, psTAXA_ECM, psMETA_ECM)
...
write.csv(difabund_ECM_df, 'output/ANCOM_ECM.csv', row.names = FALSE)

##### (1c) EM Fungi: Exploration type #####

# Generate phyloseq object
taxa_ECM_explore <- read.csv('data/statistics/taxa_ECM.csv',
                          header = TRUE, row.names = 'OTU_ID') %>%
  select(genus = ectomycorrhiza_exploration_type) %>%
  as.matrix()
psTAXA_ECM_explore <- tax_table(taxa_ECM_explore)
psEM_explore <- phyloseq(psOTU_ECM, psTAXA_ECM_explore, psMETA_ECM)
...
# Summarise statistics
difabund_ECM_df_exploration_type <- outEM_explore$res %>%
...
write.csv(difabund_ECM_df_exploration_type, 
...
relabund_AM <- 
  (relabund(read.csv('data/statistics/otu_AM.csv',
  ...
  inner_join(read.csv('data/statistics/taxa_AM.csv', header = TRUE),
             by = 'OTU_ID') %>%
...
relabund_ECM <- 
  (relabund(read.csv('data/statistics/otu_ECM.csv',
                     header = TRUE, row.names = 'OTU_ID')) * 100) %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column(var = 'sample') %>%
  as_tibble() %>%
  pivot_longer(-sample, names_to = 'OTU_ID', values_to = 'rel_abund') %>%
  inner_join(read.csv('data/statistics/taxa_ECM.csv', header = TRUE),
             by = 'OTU_ID') %>%
  select(sample, taxon = genus, rel_abund) %>%
  group_by(taxon)%>%
  summarise(rel_abund = round(sum(rel_abund), digits = 1))
...
daEMplot <- relabund_ECM %>%
  inner_join(difabund_ECM_df, by = 'taxon') %>%
  filter(rel_abund > 2) %>%
  mutate(ecotype = case_when(
    lfc > 0 ~ 'Upland',
    lfc < 0 ~ 'Coastal')
